...
const getLanguages= async (req,res)=>{
    try {
        ...
        const result = await connection.query("SELECT id,name,programmers,category FROM language");
        res.json(result); 
    } catch (error) {
        res.status(500);
...
const getLanguage= async (req,res)=>{
    try {
        const { id } = req.params;
        const connection = await getConnection();
        const result = await connection.query("SELECT id,name,programmers,category FROM language WHERE id = ?",id);
        res.json(result); 
    } catch (error) {
        res.status(500);
        res.send(error.message);
...
const addLanguage= async (req,res)=>{
    try {
        ...
        if (name===undefined||programmers===undefined) {
            res.status(400).json({ message: "Bad Request. Please fill all file." });
        }
